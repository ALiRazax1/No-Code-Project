...
//   4. Playback Speed — 0.75× / 1× / 1.25× / 1.5×
//   5. Subtitle Size  — font size slider
//
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-7">
              ...
              {PROVIDERS.map(p => (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      borderColor: settings.provider === p.id ? '#6366f1' : '#2a2a42',
                    }}
                  >
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-[#3d3d58]">{p.desc}</div>
                  </div>
                </button>
              ))}
            </div>
            ...
            <Section label="API Key">
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-xl border border-[#14142a] bg-[#060610] px-4 py-3 pr-10 font-mono text-[13px] text-[#c0c0de] placeholder-[#2a2a42] outline-none transition-all focus:border-[rgba(99,102,241,0.45)] focus:ring-2 focus:ring-[rgba(99,102,241,0.1)]"
                  autoComplete="off"
                  spellCheck={false}
                />
                <button
          ...
          {isElevenLabs && (
            <Section label="Voice IDs">
              <p className="mb-3 text-[12px] leading-relaxed text-[#3d3d58]">
                ...
                {VOICE_PROFILES.map(vp => (
                  <div key={vp.id} className="flex items-center gap-3">
                    ...
                    <div className="flex w-[72px] shrink-0 items-center gap-2">
                      ...
                      />
                      <span className="text-[12px] text-[#6b6b80]">{vp.name}</span>
                    </div>
                    ...
                    <input
                      ...
                      className="flex-1 rounded-lg border border-[#14142a] bg-[#060610] px-3 py-2 font-mono text-[11px] text-[#c0c0de] placeholder-[#22223a] outline-none transition-all focus:border-[rgba(99,102,241,0.4)] focus:ring-1 focus:ring-[rgba(99,102,241,0.15)]"
                      autoComplete="off"
                      spellCheck={false}
                    />
                  </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: settings.playbackRate === s.value
                      ? 'rgba(99,102,241,0.1)'
                      : '#0c0c18',
                    color: settings.playbackRate === s.value ? '#a5b4fc' : '#3d3d58',
                  }}
                >
                  {s.label}
                </button>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: settings.subtitleFontSize === f.value
                      ? 'rgba(99,102,241,0.1)'
                      : '#0c0c18',
                    color: settings.subtitleFontSize === f.value ? '#a5b4fc' : '#3d3d58',
                  }}
                >
                  {f.label}
                </button>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mb-3 text-[10px] font-bold uppercase tracking-[0.12em] text-[#2a2a42]">
        {label}
      </p>
...
function InfoNote({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2 rounded-lg border border-[#0e0e22] bg-[#080812] p-3">
      <Info size={12} className="mt-[1px] shrink-0 text-[#2a2a42]" />
      <p className="text-[11px] leading-relaxed text-[#2a2a42]">{children}</p>
    </div>
  )
}
